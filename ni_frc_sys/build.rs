...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::copy(
        ni_src.join("lib/chipobject/libRoboRIO_FRC_ChipObject.so.24.0.0"),
        object_folder.join("libRoboRIO_FRC_ChipObject.so"),
    )?;
    std::fs::copy(
        ni_src.join("lib/netcomm/libFRC_NetworkCommunication.so.24.0.0"),
        object_folder.join("libFRC_NetworkCommunication.so"),
    )?;
    std::fs::copy(
        ni_src.join("lib/visa/libvisa.so.23.3.0"),
