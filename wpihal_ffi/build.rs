...
use wpilib_artifact_download::{WPILIB_VERSION, NI_VERSION, WpilibArtifactInfo};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let object_dir = PathBuf::from(std::env::var_os("OUT_DIR").expect("OUT_DIR not set")).join("objects");
