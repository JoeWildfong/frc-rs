    ...
    pub fn new(can_id: i32) -> Self {
        let handle = unsafe {
            wpilib_hal_ffi::panic_on_hal_error(|status| {
                wpilib_hal_ffi::HAL_InitializeCTREPCM(
                    can_id,
...
impl PneumaticsController for CtrePcm {
    fn get_solenoids(&self) -> u32 {
        let solenoids = unsafe {
            wpilib_hal_ffi::panic_on_hal_error(|status| {
                wpilib_hal_ffi::HAL_GetCTREPCMSolenoids(self.handle, status)
            })
        };
        solenoids as u32
    }
    ...
    fn set_solenoids(&self, mask: u32, values: u32) {
        unsafe {
            wpilib_hal_ffi::panic_on_hal_error(|status| {
                wpilib_hal_ffi::HAL_SetCTREPCMSolenoids(
                    self.handle,
                    mask as i32,
                    values as i32,
                    status,
...
impl Drop for CtrePcm {
    fn drop(&mut self) {
        unsafe { wpilib_hal_ffi::HAL_FreeCTREPCM(self.handle) }
    }
}
...
impl<'a> Compressor for CtreCompressor<'a> {
    fn get_enabled(&self) -> bool {
        unsafe {
            wpilib_hal_ffi::panic_on_hal_error(|status| {
                wpilib_hal_ffi::HAL_GetCTREPCMCompressor(self.pcm.handle, status)
            }) != 0
        }
    }
    ...
    fn get_pressure_switch(&self) -> bool {
        unsafe {
            wpilib_hal_ffi::panic_on_hal_error(|status| {
                wpilib_hal_ffi::HAL_GetCTREPCMPressureSwitch(self.pcm.handle, status)
            }) != 0
        }
    }
}
